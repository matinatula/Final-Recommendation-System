...
class RecommendationWorker:
    ...
    def initialize_recommender(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize recommender: {e}", exc_info=True)
            return False
    ...
    def generate_recommendations_for_user(self, user_id: str) -> Dict:
        ...
        try:
            ...
            stored_count = self.store_recommendations(
                session, user_id, recommendations)
        ...
        try:
            ...
            if not self.hybrid_recommender:
                if not self.initialize_recommender():
                    logger.error(
                        "Failed to initialize recommender. Aborting job.")
                    return
            ...
            if results["failed_users"]:
                logger.warning(
                    f"Failed users ({len(results['failed_users'])}):")
                for failed in results["failed_users"][:10]:  # Show first 10
                    logger.warning(
                        f"  - {failed['user_id']}: {failed['error']}")
            ...
            logger.error(
                f"Fatal error in recommendation job: {e}", exc_info=True)
...
def main():
    ...
    for job in scheduler.get_jobs():
        ...
        try:
            ...
            next_run = None
        logger.info(
            f"  - {job.name} (recorereID: {job.id}) - Next run: {next_run if next_run else 'N/A'}"
        )
